...
export const addPortfolio = () => ({ type: ADD_PORTFOLIO }),
  ...
  setPortfolioSingle = portfolioSingle => ({
    type: SET_PORTFOLIO_SINGLE,
    portfolioSingle
  }),
...
export const getPortfolioRequest = (currentPage, limit) => async dispatch => {
  dispatch(setFetching(true));
  const data = await portfolioAPI.get(currentPage, limit);
  dispatch(setFetching(false));
  dispatch(setPortfolio(data));
};
export const setLikePortfolio = (id, likes) => async dispatch => {
  portfolioAPI.setLike(id, likes);
  dispatch(setPortfolioLike(id));
};
export const getPortfolioSingleRequest = url => async dispatch => {
